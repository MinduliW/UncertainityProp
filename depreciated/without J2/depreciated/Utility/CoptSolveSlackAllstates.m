function [x,changenu] = CoptSolveSlackAllstates(As,Bs,constants,xg,xgO, N,TR, xfconv, param)
% FUNCTION NAME:
%   CoptSolveSlackAllstates
%
% DESCRIPTION:
...
prob.c   = [zeros(1,6*(N+1)+3*N), ones(1, N), zeros(1, 6*(N)), 1e10*ones(1,N-1), 1e10]; %+...
...
for Nnodes = 1: N
    ...
    A = As(lbd:ubd,:);
    B = Bs(lbd:ubd, :);
    vterms = [zeros(6, 6*(Nnodes-1)), eye(6),zeros(6,6*(N-Nnodes))];
    

    dynamicsArray = [zeros(6, 6*(Nnodes-1)), A, -eye(6), zeros(6,6*(N-Nnodes)),...
        zeros(6, 3*(Nnodes-1)), B, zeros(6, 3*(N-Nnodes)), zeros(6,N), vterms, zeros(6, N)];
...
% initial position.
Aeq1 = ([eye(6), zeros(6,10*N),zeros(6, 7*N)]);
Beq1 = param.x0OscEQ';


Aeq21 =  [zeros(1,6*N), xfconv.A(1,:), zeros(1,4*N), zeros(1,6*N-6), 1 1 1 1 1 1, zeros(1,N)]; 
Beq21 = param.xfMeanKep(1) -xfconv.B(1) +  xfconv.A(1,:)*xg(end,1:6)';

Aeq22 = [zeros(1,6*N), xfconv.A(3,:), zeros(1,4*N),zeros(1,6*N-6),1 1 1 1 1 1, zeros(1,N)]; 
Beq22 =  param.xfMeanKep(3) -xfconv.B(3)+  xfconv.A(3,:)*xg(end,1:6)';

Aeq23 = [zeros(1,6*N), xfconv.A(4,:), zeros(1,4*N),zeros(1,6*N-6),1 1 1 1 1 1, zeros(1,N)];
...
prob.blx = [ prob.blx, -inf*ones(1,6*(N)), zeros(1, N)]; 
prob.bux = [prob.bux , inf*ones(1,6*(N)), inf*ones(1, N)];


%% 

prob.cones.type = zeros(2*N,1);
...
% cone for slack variables ughhhhh
l = 1:6:6*(N); 
m =  2:6:6*(N); 
n =  3:6:6*(N);
o =  4:6:6*(N); 
...
index = 10*N+6; 
for i = 1:N
    prob.cones.sub  = [prob.cones.sub 16*N+6+i index+l(i) index+m(i) index+n(i), index+o(i), index+p(i), index+q(i)]; % Add the cone constraint
end

prob.cones.subptr = [prob.cones.subptr, 4*N+1:7:(4*N+7*(N)-6)];
